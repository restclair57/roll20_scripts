...
var RecursiveTable = RecursiveTable || (function() {
  ...
  var version = '0.2.4',
    ...
    parseInlines = function(inlines,opts){
      ...
      const formatPart = (part) => (opts.showicons && part.avatar)? `<div style="${s(styles.o)}">`+`<img style="${s(styles.i)}" src="${part.avatar||clearURL}">`+(opts.iconlabel ? `<div style="${s(styles.t)}">${part.text||ch(' ')}</div>` : '')+`</div>`: part.text;

      const composeParts = (parts) => _.compose(((x)=>_.map(x,formatPart)),(opts.sort ? (x)=>_.sortBy(x,'text') : _.identity),(opts.dropempty ?  (x)=>_.filter(x,(v)=>`${v.text}${opts.showicons?v.avatar:''}`.trim().length) : _.identity))(parts).join(opts.delimiter);
